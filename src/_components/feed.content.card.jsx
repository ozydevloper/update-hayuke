...
const Description = ({ description }) => {
  ...
  return (
    <p className="text-xs w-full h-full mt-2 md:mb-0 mb-2">
      {sorted} <span className="font-bold text-primary cursor-pointer">Lihat detail...</span>
    </p>
  );
};
8
        ...
        >
          <div className="w-full flex flex-row md:flex-col justify-between items-start md:gap-y-1">
            ...
            <div className="flex items-center justify-start gap-x-2">
              <Badge className={'bg-green-200 text-green-800 font-bold border border-green-100'}>
                {biaya.name}
              </Badge>
              <Badge className={'bg-foreground text-background font-bold border border-foreground'}>
                {kalangan.name}
              </Badge>
            </div>
          </div>
